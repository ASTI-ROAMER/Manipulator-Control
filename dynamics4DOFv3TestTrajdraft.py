...
dataArduino = np.genfromtxt("ardu_testdata.txt", dtype=float, 
...
dT_ardu = dataArduino[:,5]
Tau1_ardu = dataArduino[:,6]
Tau2_ardu = dataArduino[:,7]
Tau3_ardu = dataArduino[:,8]
Tau4_ardu = dataArduino[:,9]


print("Data from Arduino")
print(dataArduino)
print(dataArduino.shape)
...
q1dotdotdes_max = 4*(q1des - q1_init)/tfinal**2
q2dotdotdes_max = 4*(q2des - q2_init)/tfinal**2
q3dotdotdes_max = 4*(q3des - q3_init)/tfinal**2
q4dotdotdes_max = 4*(q4des - q4_init)/tfinal**2
...
#   Set tc ~~~~~~
tc1 = (tf/2) - (0.5)*np.sqrt( ( ((tf**2)*q1dotdotdes_max)-4*(q1des-q1_init) ) / q1dotdotdes_max)
tc2 = (tf/2) - (0.5)*np.sqrt( ( ((tf**2)*q2dotdotdes_max)-4*(q2des-q2_init) ) / q2dotdotdes_max)
tc3 = (tf/2) - (0.5)*np.sqrt( ( ((tf**2)*q3dotdotdes_max)-4*(q3des-q3_init) ) / q3dotdotdes_max)
tc4 = (tf/2) - (0.5)*np.sqrt( ( ((tf**2)*q4dotdotdes_max)-4*(q4des-q4_init) ) / q4dotdotdes_max)

#   Set qc ~~~~~~
qc1 = (1/2)*q1dotdotdes_max*tc1+q1_init
qc2 = (1/2)*q2dotdotdes_max*tc2+q2_init
qc3 = (1/2)*q3dotdotdes_max*tc3+q3_init
qc4 = (1/2)*q4dotdotdes_max*tc4+q4_init

#   Set Velocities ~~~~~~
#Fixed

q1dotdes_max = (qm1-qc1)/(tm-tc1)
q2dotdes_max = (qm2-qc2)/(tm-tc2)
q3dotdes_max = (qm3-qc3)/(tm-tc3)
q4dotdes_max = (qm4-qc4)/(tm-tc4)
...
for p in p_index:
    ...
    if (tcurr <= tc1): #2s
        ...
        # Elevation/Angle
        q1des[0,p] = q1_init + 0.5*q1dotdotdes_max*tcurr**2

    elif (tcurr > tc1 and tcurr <= (tf-tc1)): #between #2s and #8s
        # Acceleration
        q1dotdotdes[0,p] = 0

        # Velocity
        q1dotdes[0,p] = (qm1-qc1)/(tm-tc1)

        # Elevation/Angle
        q1des[0,p] = q1_init + q1dotdotdes_max*tc1*(tcurr-tc1/2)
    else:
        q1dotdotdes[0,p] = q1dotdotdes_max
    ...
    if (tcurr <= tc2): #2s
        ...
        # Elevation/Angle
        q2des[0,p] = q2_init + 0.5*q2dotdotdes_max*tcurr**2

    elif (tcurr > tc2 and tcurr <= (tf-tc2)): #between #2s and #8s
        # Acceleration
        q2dotdotdes[0,p] = 0

        # Velocity
        q2dotdes[0,p] = (qm2-qc2)/(tm-tc2)

        # Elevation/Angle
        q2des[0,p] = q2_init + q2dotdotdes_max*tc2*(tcurr-tc2/2)
    else:
        q2dotdotdes[0,p] = q2dotdotdes_max
    ...
    if (tcurr <= tc3): #2s
        ...
        # Elevation/Angle
        q3des[0,p] = q3_init + 0.5*q3dotdotdes_max*tcurr**2

    elif (tcurr > tc3 and tcurr <= (tf-tc3)): #between #2s and #8s
        # Acceleration
        q3dotdotdes[0,p] = 0

        # Velocity
        q3dotdes[0,p] =(qm3-qc3)/(tm-tc3)

        # Elevation/Angle
        q3des[0,p] = q3_init + q3dotdotdes_max*tc3*(tcurr-tc3/2)
    else:
        q3dotdotdes[0,p] = q3dotdotdes_max
    ...
    if (tcurr <= tc4): #2s
        ...
        # Elevation/Angle
        q4des[0,p] = q4_init + 0.5*q4dotdotdes_max*tcurr**2

    elif (tcurr > tc4 and tcurr <= (tf-tc4)): #between #2s and #8s
        # Acceleration
        q4dotdotdes[0,p] = 0

        # Velocity
        q4dotdes[0,p] = (qm4-qc4)/(tm-tc4)

        # Elevation/Angle
        q4des[0,p] = q4_init + q4dotdotdes_max*tc4*(tcurr-tc4/2)
    else:
        q4dotdotdes[0,p] = q4dotdotdes_max
        ...
        # Elevation/Angle
        q4des[0,p] = q4des_main - 0.5*q3dotdotdes_max*(tf-tcurr)**2
'''
print('q1des') #(5,1)
print(q1des.shape)
print('q1dotdes') #(5,5)
...
ax[0,0].plot(x, q1_store, label = "q")
q1des_mat = q1des.reshape((100, 1))
ax[0,0].plot(x, q1des_mat, label = "q_des")
ax[0,0].plot(x, q1_ardu, label = "q_ardu")
ax[0,1].plot(x, q2_store, label = "q")
q2des_mat = q2des.reshape((100, 1))
ax[0,1].plot(x, q2des_mat, label = "q_des")
ax[0,1].plot(x, q2_ardu, label = "q_ardu")
ax[0,2].plot(x, q3_store, label = "q")
q3des_mat = q3des.reshape((100, 1))
ax[0,2].plot(x, q3des_mat, label = "q_des")
ax[0,2].plot(x, q3_ardu, label = "q_ardu")
ax[0,3].plot(x, q4_store, label = "q")
q4des_mat = q4des.reshape((100, 1))
ax[0,3].plot(x, q4des_mat, label = "q_des")
ax[0,3].plot(x, q4_ardu, label = "q_ardu")

ax[1,0].plot(x, Tau1_ardu, label = "Tau_ardu")
ax[1,0].plot(x, Tau1_store, label = "Tau_py")
ax[1,1].plot(x, Tau2_ardu, label = "Tau_ardu")
ax[1,1].plot(x, Tau2_store, label = "Tau_py")
ax[1,2].plot(x, Tau3_ardu, label = "Tau_ardu")
ax[1,2].plot(x, Tau3_store, label = "Tau_py")
ax[1,3].plot(x, Tau4_ardu, label = "Tau_ardu")
ax[1,3].plot(x, Tau4_store, label = "Tau_py")
...
ax[1,0].set_title('Tau1')
ax[1,1].set_title('Tau2')
ax[1,2].set_title('Tau3')
ax[1,3].set_title('Tau4')
